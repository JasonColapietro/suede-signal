...
const BODY = `
Each audit runs the same 26 deterministic checks against three fetches: the page you submit, the origin's \`robots.txt\`, and the origin's \`llms.txt\`. No LLM calls, no sampling. The same page state scores the same.
...
Each failed check carries a suggested fix. Fixes are sorted by their lane's weight (heaviest lanes first) and the top five become your priority list. Fixing in that order moves the score fastest.
  ...
  return (
    <article>
      ...
      <p className="mt-3 text-muted">
        The full methodology behind the grade: the checks, the weights, no black box.
      </p>
      <div
      ...
      <p className="mt-10 rounded-2xl border border-accent/30 bg-accent/[0.06] px-5 py-4 text-sm">
        Failing a check? Each fix has a copy-pasteable example in the{" "}
