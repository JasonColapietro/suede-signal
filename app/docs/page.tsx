...
Paste a URL on the [home page](/) and you get a 0–100 score, an A–F grade, five lane scores, each individual check with a pass/fail and a suggested fix, and a ranked "fix these first" list. The whole audit runs in a few seconds.

## What the audit is (and isn't)

- **Deterministic.** Each check is a concrete inspection of your live page, your \`robots.txt\`, and your \`llms.txt\`. No LLM is involved in scoring, so the same page state produces the same score.
- **Read-only.** The auditor fetches public pages the way a crawler would. It does not log in, submit forms, or mutate anything.
...
The full check list, weights, and grade bands are documented in [Scoring & lanes](/docs/scoring), and each fix is explained with copy-pasteable examples in the [Fix guide](/docs/fixes).
  ...
  return (
    <article>
      ...
      <div className="mt-10 grid gap-3 sm:grid-cols-2">
        {[
          { href: "/docs/scoring", title: "Scoring & lanes", desc: "The 26 checks, weights, and grade bands." },
          { href: "/docs/fixes", title: "Fix guide", desc: "Copy-pasteable fixes for each failing check." },
